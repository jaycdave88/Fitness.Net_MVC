...
$(document).ready(function ($) {
    ...
    $('#form-calendar .time-radio').on('click', function (e) {
        $("#modal-time").val($(this).find("figure").text());
    });
    ...
    $("#form-calendar button").on("click", function () {
        $("#form-calendar").validate({
            submitHandler: function () {
                $.post("assets/external/contact_calendar.php", $("#form-calendar").serialize(), function (response) {
                    $('#form-calendar #form-calendar-status').html(response);
                    $('#form-calendar button').attr('disabled', 'true');
                });
                return false;
            }
        });
