...
(function ($) {
        ...
        animateTheCaptions: function (nextli, opt, recalled, mtl) {
            ...
            if (allcaptions)
                jQuery.each(allcaptions, function (i) {
                    var _nc = jQuery(this);
                    punchgs.TweenLite.set(_nc.find('.tp-videoposter'), {
                        autoAlpha: 1
                    });
                    punchgs.TweenLite.set(_nc.find('iframe'), {
                        autoAlpha: 0
                    });
                })
	...
	***************************************/
        animateSingleCaption: function (_nc, opt, offsetx, offsety, i, recalled, triggerforce) {
            ...
            if (opt.debugMode) {
                _nc.closest('li').find('.helpgrid').css({
                    top: offsety + "px",
                    left: offsetx + "px"
                });
            ...
            if (_nc.hasClass("tp-videolayer") || _nc.find('iframe').length > 0 || _nc.find('video').length > 0) {
                ...
                if (ncobj.lineHeight == "auto")
                    ncobj.lineHeight = ncobj.fontSize + 4;
            ...
            $progress = _nc.data('timeline') !== undefined ? _nc.data('timeline').progress() : 0
              ,
            tl = _nc.data('timeline') || new punchgs.TimelineLite({
            ...
            if ($progress < 1 && _nc.data('outstarted') != 1 || staticdirection == 2 || triggerforce) {
                ...
                if (_nc.data('splitin') != undefined && _nc.data('splitin').match(/chars|words|lines/g) || _nc.data('splitout') != undefined && _nc.data('splitout').match(/chars|words|lines/g)) {
                    ...
                    _nc.data('mySplitText', new punchgs.SplitText(splittarget, {
                ...
                if (_nc.data('mySplitText') !== undefined && _nc.data('splitin') && _nc.data('splitin').match(/chars|words|lines/g))
                ...
                if (staticdirection != 2 || triggerforce) {
                    ...
                    if ($mask_frm) {
                        ...
                        tl.add(punchgs.TweenLite.set(_mw, {
                            overflow: "visible"
                        }, $from.elemdelay), 0);
                    }
                }
...
**********************************************************************************************/
        ...
        if (customarray)
            jQuery.each(customarray, function (index, pa) {
                ...
                if (v != undefined) {
                    ...
                    if (w == "x")
                        o.anim.x = animDataTranslator(v, o.anim.x);
                    if (w == "y")
                        o.anim.y = animDataTranslator(v, o.anim.y);
                    if (w == "z")
                        o.anim.z = animDataTranslator(v, o.anim.z);
    ...
    var calcCaptionResponsive = function (nc, opt, level, responsive) {
        ...
        // IE8 FIX FOR AUTO LINEHEIGHT
        if (obj.lineHeight == "auto")
            obj.lineHeight = obj.fontSize + 4;
        ...
        if (!nc.hasClass("tp-splitted")) {
            ...
            if (hashover)
                punchgs.TweenLite.set(nc, obj.styleProps);
    ...
    var callCaptionLoops = function (el, factor) {
        ...
        if (el.hasClass("rs-pendulum")) {
            if (el.data('loop-timeline') == undefined) {
                ...
                el.data('loop-timeline').append(new punchgs.TweenLite.fromTo(el, speed, {
                    force3D: "auto",
                    rotation: startdeg,
